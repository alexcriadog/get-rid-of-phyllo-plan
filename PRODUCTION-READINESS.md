...
- [~] `/metrics` (Prometheus) endpoint — **shipped + verified in prod 2026-06-02** on an internal ops port (9464, not Caddy-routed) across all 3 processes: default Node metrics + `http_request_duration_seconds` + connector gauges (`connector_accounts_needs_reauth`, `connector_tokens_expiring_24h`, `connector_sync_queue_waiting`). Metric names match the org obs stack's existing alert rules (HighErrorRate/HighP95Latency/NodeJsHeapHigh/EventLoopBlocked → MS Teams), so those fire for the connector with no new config. **Remaining: the transport** — the org runs a central Grafana/Prometheus(remote_write)/Alertmanager EC2 (`ec2-54-198-137-104`, netbird mesh); the connector box has no metrics agent yet, so its `/metrics` isn't ingested. PENDING a networking decision on how to connect the connector box (agent + netbird vs other). Alerting itself is then mostly free (existing rules + Teams).
