...
- [x] `/metrics` (Prometheus) + alerting — **shipped + verified end-to-end in prod 2026-06-02**.
  - Endpoint: internal ops port (9464, not Caddy-routed) on all 3 processes — default Node metrics + `http_request_duration_seconds` + connector gauges (`connector_accounts_needs_reauth`, `connector_tokens_expiring_24h`, `connector_sync_queue_waiting`).
  - Transport: replicated the org's standard per-EC2 agent stack (`tools/observability-agent/`: prom/prometheus `--agent` + node-exporter + promtail). Scrapes `:9464` on the connector app network, `remote_write`s to the central Prometheus over the VPC private IP (`172.31.43.195:9090`), promtail → Loki (`:3100`). Deployed on the connector EC2 (`docker-compose.agent.yml`).
  - Verified in the central Prometheus: `up{instance_name="connector-prod"}=1` for api/worker/scheduler; connector gauges present. Metric names match the existing alert rules (env=production, grouped by job) → HighErrorRate/HighP95Latency/NodeJsHeapHigh/EventLoopBlocked fire for the connector → MS Teams, no new rule config.
