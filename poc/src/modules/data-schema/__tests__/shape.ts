// Contract-diff helpers. The InsightIQ compatibility guarantee is structural:
// our output must carry EVERY key InsightIQ returns (we may add additive keys,
 ...
 * side is null (InsightIQ nulls a field we populate or vice versa — both fine).
...
export function diffShape(
  ...
  if (et === "object") {
    ...
    for (const k of Object.keys(eo)) {
      if (!(k in ao)) {
        problems.push(`${path}${k}: MISSING (InsightIQ has it, we don't)`);
        continue;
      }
  ...
  if (et === "array") {
    ...
    // Only compare element shape when InsightIQ's sample has an element.
  ...
  if (et !== "null" && at !== "null" && et !== at) {
    problems.push(`${path.replace(/\.$/, "")}: type ${at}, InsightIQ ${et}`);
  }
  return problems;
}
