...
import type { ApiProfile, ApiReputation } from "../api-types";
import { apiProfileId } from "../ids";
import { buildEnvelope } from "./envelope.mapper";

// Core fields we map to first-class InsightIQ profile fields; everything else on
// ProfileData is platform-specific extra (kept off the InsightIQ shape, served
// only via additive keys if ever needed).
const REPUTATION_DEFAULT: ApiReputation = {
  follower_count: null,
...
/** ProfileData → InsightIQ profile document (§4.1). */
export function toApiProfile(
  ctx: SchemaContext,
  profile: ProfileData,
): ApiProfile {
  const id = apiProfileId(ctx.accountPk);
