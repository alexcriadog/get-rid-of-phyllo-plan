// Emits InsightIQ (InsightIQ)-compatible thin webhooks to endpoints whose
// `format === 'standard'`. Thin = ids only; the consumer fetches data from our
// InsightIQ-compatible read API. See PLAN-canonical-data-api.md §5.
//
// Differences from the native emitter (OutboundWebhooksService):
//   - InsightIQ event names + envelope {event, name, id, data:{...}}.
//   - account_id/user_id/profile_id/items are minted InsightIQ UUIDs.
//   - ADDED vs UPDATED resolved via a per-(account,event) marker.
//   - Always immediate (InsightIQ has no digest cadence).
...
@Injectable()
export class StandardWebhookEmitter {
  private readonly logger = new Logger(StandardWebhookEmitter.name);
  ...
  /**
   * Fire a InsightIQ data event for a product sync that added items. `sampleIds`
  ...
  async fireData(args: {
    ...
    if (!spec) return; // product not part of the InsightIQ surface
    const acc = await this.loadAccount(args.accountId);
    if (!acc || acc.isTest) return;
    const endpoints = await this.standardEndpoints(acc.workspaceId);
  ...
  /** Fire a InsightIQ lifecycle event (connect/disconnect/session expired). */
  ...
  private buildData(
    ...
    const base: Record<string, unknown> = {
      account_id: apiAccountId(acc.pk),
      user_id: apiUserIdOrFallback(acc.endUserId, acc.pk),
      last_updated_time: naiveUtc(new Date()),
    };
    if (spec.kind === "profile") base.profile_id = apiProfileId(acc.pk);
  ...
  private async enqueue(
    ...
    try {
      ...
      this.logger.error(
        `standard webhook enqueue failed (event=${eventName}): ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  private async standardEndpoints(
    workspaceId: string,
  ): Promise<Array<{ id: string; events: unknown }>> {
    return this.prisma.webhookEndpoint.findMany({
      where: { workspaceId, active: true, format: "standard" },
  ...
  private async resolveAddedUpdated(
    ...
    const res = await this.mongo.getCollection("webhook_emit_state").updateOne(
  ...
  /**
   * Map sample ids to InsightIQ content UUIDs. For content products the sample
  ...
  private async resolveItems(
    ...
    if (product !== "comments") {
      return sampleIds.map((extId) => apiContentId(accountPk, extId));
    }
    const rows = await this.mongo
      .getCollection<{ content_external_id: string }>("comments")
    ...
    return contentExtIds.map((extId) => apiContentId(accountPk, extId));
  }
}
