// Basic-auth credential store for the InsightIQ-compatible read API.
// The consumer authenticates exactly like InsightIQ:
...
export class ApiCredentialsService {
  ...
  async verify(clientId: string, clientSecret: string): Promise<string | null> {
    const row = await this.prisma.apiCredential.findUnique({
      where: { clientId },
    });
