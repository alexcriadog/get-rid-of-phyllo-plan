// Resolves InsightIQ account/user UUIDs back to our internal rows, scoped to the
// authenticated workspace (tenancy enforcement). The InsightIQ ids are
...
import { apiAccountId, apiUserIdOrFallback } from "@modules/data-schema";
...
export class ApiAccountResolver {
  ...
  async byAccountUuid(
    ...
    return (
      rows.find((r) => apiAccountId(r.id.toString()) === accountUuid) ?? null
    );
  }

  /** Distinct end-users in the workspace, mapped to InsightIQ user uuids. */
  async usersFor(
    workspaceId: string,
  ): Promise<
  ...
  > {
    ...
    for (const r of rows) {
      ...
      const uuid = r.endUserId
        ? apiUserIdOrFallback(r.endUserId, r.id.toString())
        : apiUserIdOrFallback(null, r.id.toString());
