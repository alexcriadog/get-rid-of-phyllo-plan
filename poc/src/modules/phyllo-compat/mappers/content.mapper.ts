...
import type {
  PhylloContent,
  PhylloContentAudience,
  PhylloContentInsights,
  PhylloEngagement,
  PhylloEngagementAdditionalInfo,
  PhylloGenderAgeBucket,
} from "../phyllo-types";
import { phylloContentId } from "../ids";
...
const ENGAGEMENT_DEFAULT: PhylloEngagement = {
  like_count: null,
  dislike_count: null,
  comment_count: null,
...
): PhylloEngagementAdditionalInfo | null {
...
function buildEngagement(content: ContentData): PhylloEngagement {
...
function buildContentAudience(
  ...
  const deepGenderAge: PhylloGenderAgeBucket[] = (
  ...
  const genderAge = [
    ...genderAgeToPhyllo(genders, undefined),
    ...deepGenderAge,
  ];

  const out: PhylloContentAudience = {
    countries: countriesToPhyllo(allCountries),
    cities: citiesToPhyllo(cities),
    gender_age_distribution: genderAge,
    audience_types: toPercentPairs(types),
    gender_distribution: gendersToPhyllo(genders),
    age_distribution: [],
  };
  const empty =
...
function buildInsights(
  ...
  const viewerDemographics: PhylloGenderAgeBucket[] = (
    deep?.item?.demographics ?? []
  ).map((d) => ({
    gender: d.gender.toUpperCase(),
    age_range: d.ageGroup.replace(/^age/i, ""),
 ...
 * fold step to patch existing phyllo_contents docs (YouTube). Returns nulls
...
/** ContentData → Phyllo content document (§4.2 + §4.5 + §4.6). */
export function toPhylloContent(
  ctx: PhylloContext,
  content: ContentData,
  deep?: DeepJoin,
): PhylloContent {
  const external = content.platformContentId;
  const id = phylloContentId(ctx.accountPk, external);
...
function deriveTitle(content: ContentData): string | null {
  ...
  // most platforms Phyllo puts the caption excerpt in `title`. We mirror the
