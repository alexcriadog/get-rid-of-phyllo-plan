/**
 * Backfills the Phyllo-compatible projection (phyllo_* collections) from the
 * existing internal collections. Phase 1 of PLAN-phyllo-schema-alignment.md.
 *
 * Replays:
 *   identity_snapshots        → phyllo_profiles
 *   audience_snapshots        → phyllo_audience
 *   posts                     → phyllo_contents
 *   comments                  → phyllo_comments
 *   engagement_deep_snapshots → folded into phyllo_contents (insights/audience)
 *
 * Idempotent — uses the same deterministic UUIDv5 ids + upsert keys as the
 * live dual-write (PhylloDualWriteService), so re-runs converge. The fold step
 * runs AFTER contents so it patches docs that already exist.
 *
 * Usage:
 *   cd poc && npx ts-node -r tsconfig-paths/register scripts/backfill-phyllo-projection.ts
 *   cd poc && DRY_RUN=1 ACCOUNT_ID=13 npx ts-node -r tsconfig-paths/register scripts/backfill-phyllo-projection.ts
 */
...
import {
  ...
  toPhylloComment,
  type PhylloContext,
} from '../src/modules/phyllo-compat';
import { deepToContentParts } from '../src/modules/phyllo-compat/mappers/content.mapper';
...
function ctxOf(a: AccountRow): PhylloContext {
  return {
...
async function run(): Promise<void> {
  ...
  for await (const snap of db.collection('identity_snapshots').find(accFilter)) {
    ...
    if (!acc) continue;
    const doc = toPhylloProfile(ctxOf(acc), snap.data as ProfileData);
    if (!dryRun) {
      await db.collection('phyllo_profiles').updateOne(
  ...
  for await (const snap of db.collection('audience_snapshots').find(accFilter)) {
    const acc = byPk.get(String(snap.account_id));
    if (!acc) continue;
    const doc = toPhylloAudience(ctxOf(acc), snap.data as AudienceData);
    if (!dryRun) {
      await db.collection('phyllo_audience').updateOne(
        { account_pk: acc.id.toString() },
        { $set: { id: doc.id, account_pk: acc.id.toString(), doc, updated_at: now }, $setOnInsert: { created_at: now } },
        { upsert: true },
      );
  ...
  for await (const row of db.collection('posts').find(accFilter)) {
    ...
    const doc = toPhylloContent(ctxOf(acc), data);
    ops.push({
      updateOne: {
    ...
    tally.contents++;
    if (ops.length >= 500) await flush('phyllo_contents');
  }
  await flush('phyllo_contents');
  ...
  for await (const row of db.collection('comments').find(accFilter)) {
    ...
    if (!externalId) continue;
    const doc = toPhylloComment(ctxOf(acc), data);
    ops.push({
      updateOne: {
    ...
    tally.comments++;
    if (ops.length >= 500) await flush('phyllo_comments');
  }
  await flush('phyllo_comments');
