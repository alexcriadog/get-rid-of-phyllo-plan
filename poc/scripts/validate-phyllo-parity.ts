/**
 * Golden-diff our Phyllo-compatible API against LIVE Phyllo for the same
 * creator (PLAN-phyllo-schema-alignment.md, Phase 4). For each resource it
 * asserts our response carries EVERY key Phyllo returns (additive keys are
 * fine; missing ones are reported).
 *
 * Env:
 *   PHYLLO_BASE        e.g. https://api.staging.insightiq.ai
 *   PHYLLO_CLIENT_ID / PHYLLO_CLIENT_SECRET
 *   PHYLLO_ACCOUNT_ID  account id on Phyllo's side
 *   OURS_BASE          e.g. http://localhost:3000/phyllo   (mounts /v1/*)
 *   OURS_CLIENT_ID / OURS_CLIENT_SECRET
 *   OURS_ACCOUNT_ID    the SAME creator's account id on our side
 *
 * Usage:
 *   cd poc && npx ts-node scripts/validate-phyllo-parity.ts
 */
...
// Report keys present in `expected` (Phyllo) but missing in `actual` (ours).
...
async function main(): Promise<void> {
  const P = process.env;
  const need = ['PHYLLO_BASE', 'PHYLLO_CLIENT_ID', 'PHYLLO_CLIENT_SECRET', 'PHYLLO_ACCOUNT_ID', 'OURS_BASE', 'OURS_CLIENT_ID', 'OURS_CLIENT_SECRET', 'OURS_ACCOUNT_ID'];
  const miss = need.filter((k) => !P[k]);
  if (miss.length) { console.error('Missing env:', miss.join(', ')); process.exit(1); }

  const pAuth = basic(P.PHYLLO_CLIENT_ID!, P.PHYLLO_CLIENT_SECRET!);
  const oAuth = basic(P.OURS_CLIENT_ID!, P.OURS_CLIENT_SECRET!);
  const pAcc = P.PHYLLO_ACCOUNT_ID!, oAcc = P.OURS_ACCOUNT_ID!;
